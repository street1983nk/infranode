...
export const GET: APIRoute = async ({ site }) => {
  ...
  lines.push(`- [MCP-Server](${base}/mcp/): Gehosteter MCP-Server unter https://mcp.infranode.dev/mcp (Remote, Streamable HTTP, keylos) mit 63 Tools inkl. Stadt-Überblick (Katalog aller Datenarten), Echtzeit-Abfahrten, Städte- und Quellenübersicht. Als Connector in Claude/ChatGPT verbinden, keine Installation.`);
  ...
  lines.push(`- [MCP server](${base}/en/mcp/): Hosted MCP server at https://mcp.infranode.dev/mcp (remote, Streamable HTTP, key-free) with 63 tools incl. city overview (catalog of all data types), live departures, cities and sources overview. Add as a connector in Claude/ChatGPT, no install.`);
