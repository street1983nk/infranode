...
laedt dafuer den Geo-Crosswalk-Seed (``tender_geo_crosswalk.json``) lazy +
gecacht ueber ``seeds_dir()`` (analog reiner Mapper-Seed-Load).

Reinheit: kein I/O ausser dem einmaligen, gecachten Seed-Load; kein
``httpx``/``now()``/Logging. Die NUTS-3-/PLZ-Crosswalk-Maps und der
Register-Name-Fallback sind die einzige Aufloesungsbasis (fail-closed:
...
def load_tender_crosswalk() -> dict:
    """Laedt die NUTS-3-/PLZ-Crosswalk-Maps lazy + gecacht ueber ``seeds_dir()``.
...
def match_tender_cities(
    ...
    """Bildet Buyer-Adresse + Erfuellungsorte auf ``slug -> sortierte matches`` ab.

    Reine Funktion in der Normalisierungs-Lib. Loest die Buyer-Adresse
    (-> ``buyer_city``) und jede Erfuellungsort-Adresse
    (-> ``place_of_performance``) getrennt gegen die Crosswalk-Maps + den
    Register-Name-Fallback auf; beide matches sind moeglich. Je Slug werden die
    matches dedupliziert und sortiert. Fail-closed: unaufloesbare Adressen
