"""Tracking-Beacon: cache-freier Endpunkt, der den Erstkontakt-Push auslöst.
...
erreicht das Origin NICHT, daher feuert die ``note_first_seen``-Middleware für
...
ERSTEN Mal je IP genau einen ntfy-Erstkontakt-Push (idempotent über das
Redis-seen-Set). Die Doku-Seite ruft ihn beim "Try it" zusätzlich (fire-and-forget)
auf. Antwortet absichtlich leer (204), trägt also keine Last/keine Daten.

Sicherheit: rein lesend, kein Query-/Body-Input, Host fix; läuft hinter denselben
...
async def track() -> Response:
    ...
    ``Cache-Control: no-store`` hält Cloudflare/Browser davon ab, den Beacon zu
    cachen, damit jeder Aufruf das Origin erreicht und der Erstkontakt-Push (pro IP
    einmalig) zuverlässig ausgelöst wird. ``include_in_schema=False``: interner
    Telemetrie-Pfad, kein Teil des öffentlichen Datenvertrags.
    """
