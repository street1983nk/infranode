...
Beweist den keylosen DATEX-II-V2-Parse-Pfad ueber den landesweiten BW-Feed von
MobiData BW (Verkehrsministerium Baden-Wuerttemberg), BBox-gefiltert auf Stuttgart.
...
Daher PFLICHT-Haertung OHNE neue Dependency (T-9-01):

1. Pre-Parse-Guard: die Response-Bytes werden VOR dem Parsen auf ``<!DOCTYPE`` und
   ``<!ENTITY`` geprueft; bei Fund wird der Body abgelehnt (``ValueError``), BEVOR
   ``iterparse`` ihn sieht. Damit findet nie eine Entity-Expansion statt.
2. Size-Cap ``_MAX_BYTES``: ein Body groesser als der Cap wird nicht geparst
   (Begrenzung der ~20-MB-Variante / DoS-Schutz).
3. ``# noqa: S314, S405``: der Import von ``xml.etree.ElementTree`` triggert S405,
   der ``iterparse``-Call S314. Beide werden bewusst per noqa unterdrueckt, weil
   ...
   Charakter (anders als mastr.py) ist genau der Grund fuer den Guard.

Sicherheit (T-9-02 SSRF): der Host ist in ``_BASE`` hartkodiert; kein User-Input
gelangt in die URL. Der Adapter ist rein gegenueber Pydantic/Resilienz: er baut
...
# Host hartkodiert (T-9-02 SSRF): nur diese eine oeffentliche MobiData-Instanz.
...
# Size-Cap (T-9-01 DoS): konservativ ueber der erwarteten ~20-MB-Variante. Ein
# groesserer Body wird gar nicht erst geparst, sondern als no_data behandelt.
...
def parse_mobidata_datex2(
    ...
    *,
    ...
    haertet sie (Pre-Parse-Guard + Size-Cap) und parst sie streaming via stdlib
    ``iterparse``. Je ``situationRecord`` werden ``pointCoordinates``
    (``latitude``/``longitude``) gelesen; nur Eintraege innerhalb der Bounding-Box
    um (``lat``, ``lon``) passieren den ``_within_bbox``-Filter.

    Haertung (T-9-01, untrusted Live-Feed):
    - Pre-Parse-Guard: ``<!DOCTYPE`` / ``<!ENTITY`` im Body -> ``ValueError`` BEVOR
      ``iterparse`` laeuft (verhindert XXE / Billion-Laughs, stdlib-only).
    - Size-Cap ``_MAX_BYTES``: ein zu grosser Body -> ``ValueError`` (kein Parse).

    Rueckgabe: ``{"slug": slug, "events": [...]}``; eine leere
    ``SituationPublication`` liefert ``events == []``.
    """
    # Size-Cap (T-9-01): zu grosse Bodies gar nicht erst parsen.
    ...
    # noqa S314: stdlib-Parse bewusst gewaehlt (Decision 1, stdlib-only). Die
...
def _extract_point(record) -> tuple[float, float] | None:
    ...
    NS-robust per ``_localname`` (kein NS-Praefix noetig). Liefert ``None``, wenn
    keine validen Koordinaten gefunden werden (ein Datenfehler faellt damit aus
    dem Filter, statt einen 500 auszuloesen).
    """
...
async def fetch_mobidata_road_events(
    ...
    *,
    ...
    Live-Pfad (untrusted): laedt den hartkodierten ``_BASE`` + ``_RESOURCE``,
    erzwingt ``raise_for_status`` (5xx -> ``httpx.HTTPError`` an die Fassade ->
    STALE-ON-ERROR) und gibt die Bytes an den gehaerteten ``parse_mobidata_datex2``
    weiter (Size-Cap + Pre-Parse-Guard + iterparse). Ein abgelehnter Body
    (DOCTYPE/ENTITY oder zu gross) liefert ``no_data`` (leere events), statt eine
    Entity-Expansion zu riskieren.

    Rueckgabe-Keys (exakt was ``map_mobidata_road_events`` erwartet): ``slug`` und
    ``events``.
    """
    ...
    # zu grossen Body gar nicht erst materialisieren/parsen.
