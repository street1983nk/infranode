...
Der Zensus 2022 ist als 100m-Gitter über einen öffentlichen ArcGIS-FeatureServer
abfragbar (DL-DE/BY 2.0). JEDE Gitterzelle trägt die Gemeinde-AGS, daher lässt
sich die Einwohnerdichte EXAKT je Stadt aggregieren, mit EINER Server-seitigen
Statistik-Query (SUM(Einwohner) + COUNT der bewohnten Zellen über ``ags=<AGS>``),
ohne den großen Gitter-Datensatz herunterzuladen.

Sicherheit (T-SSRF/Injection): Host ist hartkodiert (``_BASE``); die ``ags`` stammt
aus dem validierten Register und wird zusätzlich auf reine Ziffern geprüft, bevor
...
async def fetch_population_density(
    ...
    *,
    ...
    nicht-ziffrige AGS löst ein ``ValueError`` aus (Injection-Schutz), bevor eine
    Query läuft. ``map_population_density`` errechnet daraus Fläche + Dichte.
    """
