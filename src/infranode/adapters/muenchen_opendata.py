...
WICHTIG: ``srsName=EPSG:4326`` wird angehängt, sonst liefert der GeoServer
EPSG:25832 (UTM).
...
``strasse_hausnr`` (statt straße), ``betroffene_bereiche`` (statt abschnitt).
Die Geometrie ist Polygon ODER MultiPolygon; für ``lat``/``lon`` wird defensiv
die erste Ring-Koordinate als repräsentativer Punkt genommen (Koordinaten
...
als ``httpx.HTTPError`` an die Fassade durchschlägt und der STALE-ON-ERROR-Pfad
greift.
...
Feldern; die Punkt-Extraktion fängt jede abweichende Geometrie-Struktur ab.
...
# CKAN-Host hartkodiert (T-9-02 SSRF): nur das Münchner Open-Data-Portal.
...
# [VERIFIED 2026-06-10] CKAN-Paket der Münchner Baustellen (package_show
# bestätigt, license_id dl-by-de/2.0, Mobilitätsreferat). Die alte
# datastore_search-Ressourcen-ID "baustellen-muenchen" war tot (404).
_PACKAGE_ID = "baustellen_4_weeks_opendata"

# [VERIFIED 2026-06-10] Format-Match: das Paket führt genau eine Ressource mit
# format "GeoJSON" (WFS-GetFeature mit outputFormat=application/json).
_RESOURCE_FORMAT = "geojson"

# [VERIFIED 2026-06-10] WICHTIG: die Ressourcen-URL trägt KEIN srsName; ohne
...
_FIELD_STRASSE = "strasse_hausnr"  # [VERIFIED 2026-06-10] Straße + Hausnummern
...
def _representative_point(geometry: dict) -> tuple[float | None, float | None]:
    """Liefert (lon, lat) als repräsentativen Punkt einer GeoJSON-Geometrie.
...
async def fetch_muenchen_road_events(
    ...
    *,
    ...
    """Holt München-Baustellen/Sperrungen über den CKAN-2-Step-Pfad.

    Step 1: GET ``{_BASE}/api/3/action/package_show?id={_PACKAGE_ID}`` und aus
    ``result.resources`` per ``format``-Match die GeoJSON-Ressource wählen,
    ...
    gegen ``_ALLOWED_HOSTS`` prüfen (genesis.py-Muster, sonst ``ValueError``,
    T-9-02), ``srsName=EPSG:4326`` anhängen (sonst UTM!), dann GET und aus
    ...
    Rückgabe-Keys (exakt das, was ``map_muenchen_road_events`` erwartet):
    ``slug`` und ``events``.
    """
    ...
    # GeoJSON-Ressource per Format-Match wählen ([VERIFIED 2026-06-10]).
    ...
    # T-9-02 SSRF: die entdeckte Ressourcen-URL gegen die Allowlist prüfen
    ...
    # [VERIFIED 2026-06-10]: srsName=EPSG:4326 anhängen, sonst kommt das
    # Service-CRS EPSG:25832 (UTM) und lat/lon wären Unsinn.
...
# --------------------------------------------------------------------------- #
# Parkhäuser München (DATA-40, statischer Standort-Katalog, Tier A DL-DE/BY)  #
# --------------------------------------------------------------------------- #

# [VERIFIED 2026-06-23] CKAN-Paket der Münchner Parkhäuser (package_show:
# license_id dl-by-de/2.0, Autor muenchen.de). Genau eine JSON-Ressource mit
# einem Array von Parkhaus-Orten (Standortkatalog, KEINE Live-Belegung; München
# veröffentlicht keine offene Echtzeit-Belegung). Felder je Ort [VERIFIED
...
async def fetch_muenchen_parking(
    ...
    *,
    ...
    """Holt den Münchner Parkhaus-Standortkatalog über den CKAN-2-Step-Pfad.

    Step 1: ``package_show?id=parkhaeuser-munchen`` -> aus ``result.resources``
    die JSON-Ressource (``format``-Match) wählen. Step 2: deren ``url`` gegen
    ``_ALLOWED_HOSTS`` prüfen (SSRF, T-9-02), dann GET und je Ort ein schlankes
    ...
    genutzt (München liefert den kompletten Stadt-Datensatz).

    Rückgabe-Keys (exakt das, was ``map_muenchen_parking`` erwartet): ``slug``
    und ``facilities``.
    """
    pkg_resp = await http.get(
        f"{_BASE}/api/3/action/package_show",
    ...
    # T-9-02 SSRF: entdeckte Ressourcen-URL gegen die Allowlist prüfen.
    discovered_host = urlsplit(resource_url).hostname
    if discovered_host not in _ALLOWED_HOSTS:
        raise ValueError(
            f"entdeckte Ressourcen-URL nicht in der Allowlist: {resource_url!r}"
