...
die relevanten ``properties`` als schlankes Event-dict übernommen.

Sicherheit (T-9-02 / T-05-13 SSRF): Der Host ist in ``_BASE`` hartkodiert
(nur diese eine öffentliche VIZ-Instanz), nie aus User-Input. Der ``slug``
...
liest sie nicht aus; er übernimmt ausschließlich die schlanken ``properties``.
...
# Host hartkodiert (T-9-02 / T-05-13 SSRF): nur diese eine öffentliche VIZ-Instanz.
...
async def fetch_berlin_road_events(
    ...
    *,
    ...
    Iteriert über die GeoJSON-``features`` und übernimmt je Feature die
    ...
    daher kein BBox-Filter (``lat``/``lon`` nur für Signatur-Konsistenz mit den
    übrigen Stadt-Adaptern).

    Rückgabe-Keys (exakt das, was ``map_berlin_road_events`` erwartet):
    ``slug`` und ``events``.
    """
